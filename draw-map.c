...
void LoadMapToMatrix(const char *filename, int mat[MAT_HEIGHT][MAT_WIDTH], EnemyManager* enemyMan, Player *p)
{
    ...
    int i=0;
    char linha[MAT_WIDTH + 2]; //Tem isso pois o arquivo l� uma linha inteira por vez. Tem o mais dois para pegar o \n que � CD LF ou alguma coisa do g�nero no arquivo oficial
        ...
        for (int x = 0; x < MAT_WIDTH; x++)
        {
            switch (linha[x])
            {
                ...
                case 'e':
                    ...
                    enemyMan->enemies[i].position={(float)x, (float)y};
                    enemyMan->enemies[i].size={(float)SQR_SIZ,(float)SQR_SIZ};
                    i++;
                    break;
                ...
                case 'P':
                case 'p':
                    p->position.y=y*SQR_SIZ;
                    p->position.x=x*SQR_SIZ;
                    p->size.x=SQR_SIZ;
                    p->size.y=SQR_SIZ;
                    break;
        ...
        for(int j = 0; j <= MAT_WIDTH; j++){
            ...
            switch(block){
                ...
                case 1:     //Blocos do mapa
                    DrawTexture(floorTex, j * SQR_SIZ, i * SQR_SIZ, WHITE);
                    break;
                case 2:     //Escada
                case 3:
                    DrawTexture(hand_stairsTex, j * SQR_SIZ, i * SQR_SIZ, WHITE);
                    break;
                case 4:
                    ...
                    break;
                case 5:
                    DrawRectangle(j * SQR_SIZ, i * SQR_SIZ, SQR_SIZ, SQR_SIZ, COLOR_GOAL);
                    break;
            }
        }
    }
